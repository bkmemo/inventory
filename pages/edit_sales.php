        ...
        <div class="box box-primary">
           ...
           <?php 
		    ...
		    while($row3 = mysqli_fetch_array($result3)){
			   ...
			   // $phone_return_status = $row3["phone_return_status"];
			   // $faulty_description = $row3["faulty_description"];
		    }
		   ...
		   ?>
              ...
              <div class="row">
					<div class="col-md-4">
                        Customer Name:
                		<input type="hidden" value="<?php echo $_SESSION['customer_name']; ?>" name="customer_name" required />
                        <input type="text"  name="customer" value="<?php echo $customer_name; ?>" class="form-control" />
                     </div>
                     <div class="col-md-4">
                         Customer Telephone:
                		<input type="text"   value="<?php echo $phone; ?>" name="customer_telephone"  class="form-control" required />
                     </div>
                     <div class="col-md-4">
                    Date Of Sale:
                    <input  id="dopDate" type="text" class="form-control" value="<?php echo date('Y-m-d'); ?>"  name="date_of_sale"  ></p>
					</div>
            </div>
		   ...
		   <table class="table table-bordered table-hover">
				...
				//$ukusedphones_onsale_query="select * from phones where ITEM_ID=".$a['ITEM_ID']." && phone_condition = 'UKUsedPhone' && phone_status ='OnSale'";
					...
					$number_of_items = mysqli_num_rows(mysqli_query($con,$query0));
					$count = 0;
					$row = 1; $row1 = 1;
					$total = 0;
                    while($result0=mysqli_fetch_assoc($query_run0)){
						...
						$querys="select s.BUY_PRICE from item_sellprice s where s.ITEM_ID='$item_id'";
						$query_runs=mysqli_query($con,$querys);
						while($results=mysqli_fetch_assoc($query_runs)){
							$item_buy_price = $results['BUY_PRICE'];
						}
						...
						echo "<tr>";
								...
								<?php
                                    ...
                                    echo "<select id='serial_".$row1."' name='serial_".$row1."[]' class='serial_multiple' onChange='change_item_price2($row1)' multiple='multiple' style='width: 200px;''>";
                                        ...
                                        while($result=mysqli_fetch_assoc($query_run)){
											...
											$item_old_serial_numbers[$index2]['serial_number']=$serial_number;
											$item_old_serial_numbers[$index2]['item_id']=$item_id;
											$index2++;
                                        }
										...
										if($query_run2){
											...
											if($count2>0){
												...
												while($a2=mysqli_fetch_assoc($query_run2)){
													echo "<option >".$a2["serial_number"]."</option>";
												}
											}
										}
                                   echo "</select>";
							...
							<td><div id="buying_price_<?php echo $row; ?>"><?php echo $item_buy_price ?></div></td>
...
$(document).ready(function() {
	...
	var number_of_items = <?php echo json_encode($number_of_items); ?>;
	for (var i=1; i<=number_of_items; i++) {
		change_item_price2(i);
	}
});
...
function total(){
	...
	function change_item_price(row_id){
		...
		for(x=0;x<item_old_serial_numbers.length;x++){
			if(item_ids[item_select.selectedIndex] == item_old_serial_numbers[x]['item_id']){
				serial_numbers = serial_numbers+"<option selected value='"+item_old_serial_numbers[x]['serial_number']+"'>"+item_old_serial_numbers[x]['serial_number']+"</option>";
			}
		}
...
// get items sell prices
	<?php
		...
		$query="select i.ITEM_ID, i.ITEM_NAME, s.PRICE, s.BUY_PRICE from items i inner join item_sellprice s on i.ITEM_ID=s.ITEM_ID inner join phones p on i.ITEM_ID=p.ITEM_ID group by i.ITEM_ID order by i.ITEM_NAME ASC";
	...
	item_old_serial_numbers = <?php echo json_encode($item_old_serial_numbers); ?>;
	

	//console.log(item_old_serial_numbers);
