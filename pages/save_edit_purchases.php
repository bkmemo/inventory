<?php
	...
	if(isset($_POST['save'])){
		...
		for($i =0; $i<count($_POST['item_name']);$i++){					  
			...
			$result34 = mysqli_query($con,"UPDATE purchased_items SET SUPPLIER_ID=(select supplier_id from suppliers where supplier_name='$supplier_name'), DATE_OF_PURCHASE='$date_of_purchase', RECEIPT_NO='$receipt_no' where  PURCHASE_ID=$purchase_id");
			$purchase_details_id = $_POST['purchase_details_id'][$i];
			$new_quantity = 0;
                        ...
                        foreach($serials as $serial_number){
		                    ...
		                    $query_run09=mysqli_query($con, $query09)or die(mysqli_error($con));
		                        ...
		                        if(mysqli_num_rows($query_run09)<=0){
		                        	$query99="insert into phones (ITEM_ID, serial_number, supplier_id, phone_condition, register_date, phone_status) values((select ITEM_ID from items where ITEM_NAME='$item_name'), '$serial_number', (select supplier_id from suppliers where supplier_name='$supplier_name'), '$phone_condition', '$date_of_purchase', 'OnSale')";
