<?php
	...
	$query="SELECT * FROM payments_received WHERE PAYMENT_RECEIVED_ID=$_GET[PAYMENT_RECEIVED_ID]";
	$result=mysqli_query($con,$query);
	$number_of_results = mysqli_num_rows($result);
	if($number_of_results>0){
		$query="DELETE FROM payments_received WHERE PAYMENT_RECEIVED_ID=$_GET[PAYMENT_RECEIVED_ID]";
		$result = mysqli_query($con,$query)or die(mysqli_error($con));
		if($result){
				echo "<strong><font color='green'>Brand Successfuly Deleted successfully</font></strong>";
                include ("show_payments_receieved.php");	
		}
		else{
