...
<?php   include ("../pages/side.php");?>        
       ...
       <?php
		   ...
		   if(isset($_POST['search'])){
				...
				$query="select * from sales inner join customer on sales.CUSTOMER_ID=customer.customer_id where sales.DATE_OF_SALE like '%$search_value%' || sales.SALES_ID like '%$search_value%'  && sales.CUSTOMER_ID=(select customer_id from customer where customer_name='$customer_name') order by sales.SALES_ID desc limit 5";
		   }
		   else{
			   $query="select * from sales inner join customer inner join user on sales.CUSTOMER_ID=customer.customer_id && sales.USER_ID=user.id where sales.CUSTOMER_ID=(select customer_id from customer where customer_name='$customer_name') order by sales.SALES_ID desc limit 5";
		   }
					   ...
					   if($customer_id == $a['CUSTOMER_ID']){
					    ...
					    echo "<table class='table table-striped'>";
							 echo "<thead>";
								...
								echo "<th> Sold By &nbsp;&nbsp;&nbsp;<font color='#FFFFFF' size='+1'><b>$username C/0 $SOLD_BY</b></font></th>";
								  echo "</tr>";
							 echo "</thead>";
							 echo "<tbody>";
							 echo "<tr>"; 
							 ...
							 echo "<tfoot>";
								  ...
								  echo "<tr>";
										...
										echo "</td>";
										echo "<td colspan=''>";
											echo "<form method='post' action='delete_sale.php' onsubmit='return confirmDelete();' >";
											//echo "<input type='hidden'  name='SALE_DETAILS_ID'/>";
											echo "<input type='hidden' value='$sale_id' name='sale_id'/>";
											echo "<input style='float:right;' class='btn btn-danger' type='submit' value='Delete Sale' />";
											echo "</form>";
										echo "</td>";
								  echo "</tr>";
