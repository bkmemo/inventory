                            ...
                            foreach($serials as $serial_number){
								if(isset($serial_number) && $serial_number != null){
									...
									if(mysqli_num_rows($query_run09)<=0){
										...
										$query99="insert into phones (ITEM_ID, serial_number,  supplier_id, phone_condition, register_date,  phone_status) values((select ITEM_ID from items where ITEM_NAME='$item_name'), '$serial_number', (select supplier_id from suppliers where supplier_name='$supplier_name'), '$phone_condition', '$purchase_date', 'OnSale')";
